...
project_snapshot_result decide_project_identity(
    const std::filesystem::path& path,
    ...
    if (persisted.change_token_available &&
        ...
        if (proof == project_token_result::missing) {
            return project_snapshot_result::missing;
        }

        if (proof == project_token_result::failed) {
            return project_snapshot_result::failed;
        }

        if (proof == project_token_result::available &&
            unchanged) {
    ...
    if (persisted.content_hash_available &&
